...
public class Item {
	...
	public static RemoteItem fromMetadata(final String remoteIdentifier, final boolean isFolder, final String name, final String metadata, Long remoteFilesize, FileTime remoteCreationtime) {
	...
	public static Item fromLocalData(String name, ItemType type, Long filesize, FileTime creationtime, FileTime modifytime, FileTime accesstime, Map<String, String[]> map) {
	...
	private static Item initItem(final Item item, final String name, final String remoteIdentifier, final String[] metadata) {
		...
		case 0:
			...
			if (permissions == null)
				map.put(ATTRIBUTE_POSIX, new String[] { group, user });
			else
				map.put(ATTRIBUTE_POSIX, new String[] { group, user, permissions.toString() });
	...
	private String[] getDataArray() {
		...
		return ArrayUtils.addAll(new String[] { METADATA_VERSION.toString(), type.toString(), filesize != null ? filesize.toString() : null, creationtime != null ? creationtime.toString() : null,
				modifytime != null ? modifytime.toString() : null, accesstime != null ? accesstime.toString() : null, checksum }, attributes);
	}
